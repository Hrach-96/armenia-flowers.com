...
class SppagebuilderAddonPerson extends SppagebuilderAddons{
	...
	public function render() {
		...
		$phone 				= (isset($this->addon->settings->phone) && $this->addon->settings->phone) ? $this->addon->settings->phone : '';
		...
		if($facebook || $twitter || $google_plus || $youtube || $linkedin || $pinterest || $flickr || $dribbble || $behance || $instagram) {
			...
			if($name || $designation || $email || $phone) {
				...
				if($email) $output  .= '<span class="sppb-persons-email"><i class="linearicons-envelope"></i><a href="mailto:' . $email . '">' . $email . '</a></span>';
				if($phone) $output  .= '<span class="sppb-persons-phone"><i class="linearicons-telephone"></i><a href="callto:' . $phone . '">' . $phone . '</a></span>';
				$output  .= '</div>';
			}
