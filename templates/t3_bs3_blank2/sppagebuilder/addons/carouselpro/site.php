...
class SppagebuilderAddonCarouselpro extends SppagebuilderAddons {
	...
	public function render() {
		...
		//Addons option
		$controller 		= (isset($this->addon->settings->controller)) ? $this->addon->settings->controller : 0;
		$nav 		= (isset($this->addon->settings->arrow)) ? $this->addon->settings->arrow : 0;
		...
		$style_pro = (isset($this->addon->settings->style_pro) && $this->addon->settings->style_pro) ? $this->addon->settings->style_pro : '';
		...
		$bg_url = (isset($this->addon->settings->title_link) && $this->addon->settings->title_link) ? $this->addon->settings->title_link : '';
		$document 	= JFactory::getDocument();
		$app =& JFactory::getApplication();
		$template = $app->getTemplate();
		$owlpath = JURI::base() .'templates/'.$template.'/sppagebuilder/addons/carouselpro/js/owl.carousel.min.js';
		$document->addScript($owlpath);

		$output  = '<div id="sppb-carousel-'. $this->addon->id .'"  class="sppb-carousel-pro sppb-slide' . $class . '  ' . $style_pro . '">';

		//$output .= '<div class="owl-carousel owl-theme sppb-carousel-inner ' . $alignment . '">';
	 	$output .= '<ul class="owl-carousel owl-theme sppb-carousel-inner"><li class="item">';
		$col = 1;
        $last = $limit;
		if(isset($this->addon->settings->sp_carouselpro_item) && count((array) $this->addon->settings->sp_carouselpro_item)){
			foreach ($this->addon->settings->sp_carouselpro_item as $key => $value) {
				...
				$contentposition = (isset($value->content_position_pro) && $value->content_position_pro) ? $value->content_position_pro : '';
				$contentwidth = (isset($value->content_width_pro) && $value->content_width_pro) ? $value->content_width_pro : '';
				$bg_url = (isset($value->bg_url) && $value->bg_url) ? $value->bg_url : '';

				$output  .= '<div id="sppb-item-' . $this->addon->id . $key . '" class="sppb-items'. $bg_class . (($key == 0) ? ' active' : '') .'">';
				if($bg_url){
					$output  .='<a class="link" href="'.$bg_url.'" target="_self">';
				...
				if((isset($value->title) && $value->title) || (isset($value->content) && $value->content) || (isset($value->title) && $value->title) || (isset($value->content) && $value->content) ) {
				$output  .= '<div class="sppb-carousel-item-inner '.$contentposition.'">';
				if($contentposition == 'text-left'){
					$output  .= '<div class="sppb-col6">';
				$output  .= '<div class="sppb-carousel-pro-text" style="width:'.$contentwidth.'%;">';
	
				if((isset($value->title) && $value->title) || (isset($value->content) && $value->content) ) {
					$output  .= (isset($value->title) && $value->title) ? '<h2 class="slide-title">' . $value->title . '</h2>' : '';
				...
				$output  .= '<div class="sppb-col6 video">';
				$output  .= '<div class="sppb-text">';
				...
				$output  .= '</div>';
				$output  .= '</div>';
				}elseif($contentposition == 'text-right') {
	...
	public function css() {
		...
		foreach ($this->addon->settings->sp_carouselpro_item as $key => $value) {
			...
			$title_css .= (isset($value->title_color) && $value->title_color) ? 'color: ' . $value->title_color . ';' : '';
	       	$title_css .= (isset($value->title_font_style) && $value->title_font_style) ? 'font-weight: ' . $value->title_font_style  . '!important; ' : '';
			...
			$content_css .= (isset($value->content_color) && $value->content_color) ? 'color:' . $value->content_color . ';' : '';
	       	$content_css .= (isset($value->content_font_style) && $value->content_font_style) ? 'font-weight: ' . $value->content_font_style  . '!important; ' : '';
...
public function js() {
		...
		if($controller){
			$controller = 'dots:true';
		}else {
			$controller = 'dots:false';
		} 
		if($nav){
			$jnav = 'nav:true';
		}else {
