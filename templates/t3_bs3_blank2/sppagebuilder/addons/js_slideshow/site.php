				...
				$output .='<div id="'.$uniqid.'" class="sp-item '. (($item_key == 0) ? ' active' : '') .' '.($content_vertical_alignment ? 'slider-content-vercally-center' : '').'">';
					...
					if($content_container_option==='bootstrap'){
						if(!$image_in_column){
							...
							$output .= '<div class="sppb-container">';
							$output .= '<div class="sppb-row-img">';
							$output .= '<div class="sppb-col-sm-12-img">';
						}
					} else {
					...
					$image_content_alignment = (isset($item_value->image_content_alignment) && $item_value->image_content_alignment) ? $item_value->image_content_alignment : '';
						...
						if(!$image_in_column){
							...
							foreach($item_value->slideshow_inner_items as $inner_item_key => $inner_value){
						        ...
						        $btn_class = (isset($inner_value->button_type) && $inner_value->button_type) ? ' sppb-btn-' . $inner_value->button_type : '';
        						$btn_class .= (isset($inner_value->button_size) && $inner_value->button_size) ? ' sppb-btn-' . $inner_value->button_size : '';
       							$btn_class .= (isset($inner_value->button_shape) && $inner_value->button_shape) ? ' sppb-btn-' . $inner_value->button_shape : ' sppb-btn-rounded';
							    $btn_class .= (isset($inner_value->button_appearance) && $inner_value->button_appearance) ? ' sppb-btn-' . $inner_value->button_appearance : '';

								if ($button_icon_position == 'left') {
									$btn_content = ($button_icon) ? '<span class="slider-btn-text"> <span class="sp-slider-btn-icon"><i class="fa ' . $button_icon . '"></i></span> ' . $btn_content . '</span>': '<span class="slider-btn-text">'.$btn_content.'</span>';
								} else {
									$btn_content = ($button_icon) ? '<span class="slider-btn-text">' . $btn_content . ' <span class="sp-slider-btn-icon"><i class="fa ' . $button_icon . '"></i></span></span>' : '<span class="slider-btn-text">'.$btn_content.'</span>';
								}
								...
								if($content_type == 'text_content'){
									...
									$output .='<a id="'.$inner_uniqid.'" '.($button_target == '_blank' ? 'target="_blank"' : '').' class="sppb-sp-slider-button sppb-btn '.$btn_class.'" href="'.$button_url.'" data-layer="true" data-animation=\'{'.$animation_settings.'}\'>';
									$output .= $btn_content;
									$output .='</a>';
									...
									$output .= '<span class="fa '.$icon_content.'"></span>';
									$output .='</span>';
							...
							if(!$image_column_reverse){
								$output .='<div class="text-column sppb-col-xs-'.($image_column_width_xs == 12 ? 12 : (12-$image_column_width_xs)).' sppb-col-sm-'.($image_column_width_sm == 12 ? 12 : (12-$image_column_width_sm)).' sppb-col-md-'.($image_column_width == 12 ? 12 : (12-$image_column_width)).'">';
								$output .='<div class="sp-slider-content-align-'.$content_alignment.'">';
								foreach($item_value->slideshow_inner_items as $inner_item_key => $inner_value){
									$inner_uniqid = 'sp-slider-inner-item-' . $increasing_addon_id .'-num-' . $inner_item_key . '-key';
			...
			foreach ($settings->slideshow_items as $item_key => $item_value) {
				...
				$css .= $addon_id . ' .dot-controller-with_image.sp-slider .sp-dots ul li.sp-dot-'.$item_key.' {';
					if(strpos($item_value->slider_img, "http://") !== false || strpos($item_value->slider_img, "https://") !== false){
						...
						$css .= 'background: url(\''.JURI::base(true). '/' .$item_value->slider_img.'\') no-repeat scroll center center / cover;';
					}
				$css .= '}';
				...
				if(isset($item_value->slideshow_inner_items)){
					foreach($item_value->slideshow_inner_items as $inner_item_key => $inner_value){
						...
						$image_content_style .= (isset($inner_value->image_content_height) && $inner_value->image_content_height->md) ? 'height: ' . $inner_value->image_content_height->md . 'px;' : 'height:auto;';
						$image_content_style .= (isset($inner_value->image_content_width) && $inner_value->image_content_width->md) ? 'width: ' . $inner_value->image_content_width->md . 'px;' : 'width:100%;';
	 ...
	 public static function getTemplate(){
		...
		#>
				...
				#>
						...
						image_content_style += (!_.isEmpty(inner_value.image_content_height) && inner_value.image_content_height.md) ? `height:${inner_value.image_content_height.md}px;` : "height:300px;";
