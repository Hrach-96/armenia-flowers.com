			...
			switch ($type) {
				...
				case 'A':
						...
						foreach ($uncatChildren as $k => $child) {
							/*if(!isset($child[$customfield->customfield_value])){
								...
								$options[] = array('value' => JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&virtuemart_category_id=' . $virtuemart_category_id . '&virtuemart_product_id=' . $productChild->virtuemart_product_id), 'text' => $productChild->{$customfield->customfield_value}.$priceStr);
									...
									$options[] = array('value' => $productChild->virtuemart_product_id, 'text' => $productChild->{$customfield->customfield_value}.$priceStr);
								}
								if($selected==$child){
									$selectedFound = true;
					...
					if($input->getCmd('action') !== 'test'){
						...
						$html .= JHtml::_ ('select.genericlist', $options, $fieldname, ' size="1" class="vm-chzn-select" data-dynamic-update="1" ', "value", "text",
						JRoute::_ ($url,false),$idTag);
					}
