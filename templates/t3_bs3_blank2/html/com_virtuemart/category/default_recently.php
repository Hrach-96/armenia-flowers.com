...
if(count($products) > 1){
	echo '<h3 class="module-title item recent'.$i.'">'.JText::_('RECENTLY_VIEWED_PRODUCTS').'</h3>';
		echo '<div class="prod_box recentproducts"><ul id="sliderrecent" class="recentproducts vmproduct layout">';
		$i=1;
...
<?php } elseif ($lblhot && $prod->product_sales > $lblhotcount) {?>
	...
	<?php } ?>   
        ...
        <div class="Price">
        <?php
				 if ($prod->prices['basePriceWithTax']>0 && $discont >0) 
                    echo '<span class="WithoutTax">' . $currency->createPriceDiv('basePriceWithTax','',$prod->prices,true) . '</span>';
