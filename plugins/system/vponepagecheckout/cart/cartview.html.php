...
----------------------------------------------------------------------------------------------------------
$Revision: 35 $
$LastChangedDate: 2015-02-08 17:34:28 +0530 (Sun, 08 Feb 2015) $
$Id: cartview.html.php 35 2015-02-08 12:04:28Z abhishekdas $
----------------------------------------------------------------------------------------------------------*/
...
class VirtueMartViewCart extends VmView 
{
	...
	public function display($tpl = null) 
	{
		...
		switch($checkoutTask)
		{
			...
			case 'getpaymentlist' :
				...
				unset($dom, $scripts, $script);
				$result = array('payments' => $payments, 'payment_script' => $_script, 'payment_scripts' => $_scripts);
				$this->jsonReturn($result);
				break;
			...
			case 'getshipmentscripts' :
				...
				unset($dom, $scripts, $script);
				$return = array('shipments' => $shipments, 'shipment_script' => $_script, 'shipment_scripts' => $_scripts);
				$this->jsonReturn($return);
				break;
				
			case 'savebtaddress' :
				setcookie('sender_name', $_POST['first_name'], time() + (14400 * 30), '/');
				setcookie('sender_phone', $_POST['phone_1'], time() + (14400 * 30), '/');
				setcookie('sender_email', $_POST['email'], time() + (14400 * 30), '/');
				setcookie('customer_note', $_POST['customer_note'], time() + (14400 * 30), '/');
			...
			case 'savestaddress' :
				setcookie('receiver_name', $_POST['shipto_last_name'], time() + (14400 * 30), '/');
				setcookie('receiver_address', $_POST['shipto_address_1'], time() + (14400 * 30), '/');
				setcookie('receiver_phone', $_POST['shipto_phone_2'], time() + (14400 * 30), '/');
				$this->prepareCartForUpdate();
				$error = false;
				...
				{
					$this->cart->ST['virtuemart_userinfo_id'] = $data['shipto_virtuemart_userinfo_id'];
				}
				//$this->cart->prepareCartData();
				$stage = isset($data['stage']) ? strval($data['stage']) : null;
				if($stage == 'final' && $this->juser->id) 
				{
					...
					$user = VmModel::getModel('user');
					if(!$user->storeAddress($data)) 
					{
						$error = true;
					}
				}
				$messages = ($stage == 'final') ? $this->getMessages(false) : $this->getMessages();
				$result = array('error' => intval($error), 'info' => $this->cart->ST, 'msg' => $messages);
				$this->jsonReturn($result);
				break;
			...
			case 'setpayment' :
				$sender_name = $_COOKIE['sender_name'];
				$sender_phone = $_COOKIE['sender_phone'];
				$sender_email = $_COOKIE['sender_email'];
				$receiver_name = $_COOKIE['receiver_name'];
				$receiver_address = $_COOKIE['receiver_address'];
				$receiver_phone = $_COOKIE['receiver_phone'];
				$customer_note = $_COOKIE['customer_note'];
				$payment_type_field = $_POST['virtuemart_paymentmethod_id'];

				unset($_COOKIE['sender_name']);
				unset($_COOKIE['sender_phone']);
				unset($_COOKIE['sender_email']);
				unset($_COOKIE['receiver_name']);
				unset($_COOKIE['receiver_address']);
				unset($_COOKIE['receiver_phone']);
				unset($_COOKIE['customer_note']);


				// send robot
				$product = '';
				foreach($this->cart->products as $cart){
					$product.= $cart->product_sku . " " . $cart->product_name . " /---/ ";
				}
				$srsinfo = '';
				if(isset($_COOKIE['srsinfo'])){
					$srsinfo = $_COOKIE['srsinfo'];
				}
				$return = $this->setPaymentMethod();
				// Remove $ icon
				$price = substr($this->currencyDisplay->priceDisplay($this->cart->cartPrices['salesPrice']), 1);
			    ...
			    $serv_data = implode(",",$access_token_parameters);
				$access_token_parameters["hash"] = md5($server_key.md5($server_key.$serv_data));
				$curl = curl_init('http://new.regard-group.ru/account/flower_orders/addRobot.php');
			    ...
			    curl_close($curl);
			    if($result != 1){
			    	// send email if robot not sending
			    	$to = "info@armenia-flowers.com";
					$subject = "Robot Not Added";
					$message = "
					<html>
					<head>
					<title>Robot Not Added</title>
					</head>
					<body>
					<p>Order Info!</p>
					<table>
			...
			case 'verifycheckout' :
				...
				else
				{
					...
					$this->cart->_inCheckOut = false;
					$this->cart->setCartIntoSession(true);
					$result = array('error' => 0, 'msg' => $this->getMessages(), 'cart' => $this->cart);
				}
				$this->jsonReturn($result);
				break;
	...
	*/
	...
	{	
		...
		$selectedAddress = isset($this->cart->selected_shipto) ? intval($this->cart->selected_shipto) : 0;
	...
	*/
	private function registerUser()
	{
		...
		if($this->params->get('email_as_username', 0) && empty($data['username']))
		{
