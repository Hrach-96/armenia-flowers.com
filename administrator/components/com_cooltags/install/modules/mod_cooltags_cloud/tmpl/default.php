...
if($cooltags_itemid=='')
	$cooltags_itemid = JFactory::getApplication()->input->get->get('Itemid','','int');
...
if(count($list)>0)
{
	...
	while (list($key, $value) = each($x))
	{
		...
		if ($value>0 && $i <= $limit && $key!=''){
			if($mode==1)
				$tag_url = JRoute::_('index.php?option=com_cooltags&view=productslist&tag='.$key.'&Itemid='.$cooltags_itemid  );
			else
				$tag_url = JRoute::_('index.php?searchword='.$key.'&ordering=newest&searchphrase=exact&option=com_search&Itemid='.$cooltags_itemid  );
