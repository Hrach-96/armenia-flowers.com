...
//$str = str_replace('plugins/system/jmsmultiupload/assets/server/php/index.php','',$_SERVER['PHP_SELF']);
//define('JPATH_BASE',$_SERVER['DOCUMENT_ROOT'].DS.$str);
...
class UploadHandler {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $db = JFactory::getDbo();
        $sql = 'SELECT config FROM #__virtuemart_configs WHERE virtuemart_config_id = 1';
        $db->setQuery($sql);
        $vm_config = $db->loadResult();
        $img = explode('|', $vm_config);
        foreach ($img AS $k => $v) {
            if (strpos($v, 'img_width') !== false) {
                $img_width = filter_var($v, FILTER_SANITIZE_NUMBER_INT);
            }
            if (strpos($v, 'img_height') !== false) {
                $img_height = filter_var($v, FILTER_SANITIZE_NUMBER_INT);
            }
        }

        $url = VmConfig::get('media_product_path', 'images/stories/virtuemart/product/');

        $this->options = array(
            'script_url' => $this->get_full_url() . '/',
            'upload_dir' => JPATH_SITE . '/' . $url,
            'upload_url' => JPATH_SITE . '/' . $url,
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    public function set_thumb_size($height, $width) {
        $this->options['image_versions'] = array('resized' => array(
                'max_width' => $height,
                'max_height' => $width
            )
        );
    ...
    protected function get_unique_filename($name, $type = null, $index = null, $content_range = null) {
        ...
        $name = preg_replace('/-+/', '_', $name); // Replaces multiple hyphens with single one.
        $name = rtrim($name, '.' . $file_type) . "_" . time() . '_' . rand(0, 1000) . '.' . $file_type;
    ...
    protected function handle_image_file($file_path, $file) {
        ...
        $db = JFactory::getDbo();
        $jms_img = new stdClass();
        $jms_img->virtuemart_vendor_id = 1;
        $jms_img->file_title = $file->name;
        $jms_img->file_mimetype = $file->type;
        $jms_img->file_type = 'product';

        
        
        $file_url = substr($file->url, strpos($file->url, 'images'));
        $file_url_thumb = substr($file->resized_url, strpos($file->resized_url, 'images'));

        $jms_img->file_url = $file_url;
        $jms_img->file_url_thumb = $file_url_thumb;
        $jms_img->published = 1;
        if (!$db->insertObject('#__virtuemart_medias', $jms_img, 'virtuemart_media_id')) {
            echo $db->stderr();
            exit;
        }
        $id = $db->insertid();
        $product_id = JRequest::getVar('virtuemart_product_id');
        $sql = 'INSERT INTO #__virtuemart_product_medias (virtuemart_product_id,virtuemart_media_id,ordering) VALUES (' . $product_id . ',' . $jms_img->virtuemart_media_id . ',1)';
        $db->setQuery($sql);
        $db->query();
