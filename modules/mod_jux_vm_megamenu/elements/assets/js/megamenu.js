...
!function ($) {
	...
	$.fn.megamenuAdmin = function (options) {
		...
		megamenu = $(this).find('.jux-megamenu');
		nav_items = megamenu.find('ul[class*="level"]>li>:first-child');
		nav_subs = megamenu.find('.jux-nav-child');
		...
		nav_items.each (function () {			
			...
			if (liitem.data ('hidesub') == 1) {
				var sub = liitem.find('.jux-nav-child:first');
		...
		$('.jux-admin-mm-row').click (function(event) {
	...
	var actions = {};
	actions.data = {};
	actions.toggleDisplay = function(){
		if (!currentSelected) return ;
		var liitem = currentSelected.closest('li');
		if(liitem.hasClass('display-toggle')){
			liitem.removeClass('display-toggle');
			liitem.data('display',1);
		}else{
			liitem.addClass('display-toggle');
			liitem.data('display',(-1));
		}
	...
	actions.toggleSub = function () {
		if (!currentSelected) return ;
		var liitem = currentSelected.closest('li'),
		sub = liitem.find ('.jux-nav-child:first');
		if (liitem.data('group')) return; // not allow do with group
		if (sub.length == 0 || sub.css('display') == 'none') {
			// add sub
			if (sub.length == 0) {
				sub = $('<div class="jux-nav-child dropdown-menu mega-dropdown-menu"><div class="row-fluid"><div class="span12" data-width="12"><div class="mega-inner"></div></div></div></div>').appendTo(liitem);
	...
	actions.toggleScreen = function () {
		if ($('.toolbox-togglescreen').hasClass('jux-fullscreen-full')) {
			$('.subhead-collapse').removeClass ('subhead-fixed');
			$('#jux-admin-megamenu').closest('.controls,li').removeClass ('jux-admin-control-fixed');			
			$('.toolbox-togglescreen').removeClass ('jux-fullscreen-full').find('i').removeClass().addClass(actions.datas.iconfull);
		} else {
			$('.subhead-collapse').addClass ('subhead-fixed');
			$('#jux-admin-megamenu').closest('.controls,li').addClass ('jux-admin-control-fixed');
			$('.toolbox-togglescreen').addClass ('jux-fullscreen-full').find('i').removeClass().addClass(actions.datas.iconsmall);
			$('.pane-sliders').css('position','static');	
		}
	}
	...
	actions.saveConfig = function (e) {
		...
		items.each (function(){
			...
			if ($this.hasClass ('mega')) {
				var $sub = $this.find ('.jux-nav-child:first');
				item['sub'] = {};
		...
		var jmmconfig = $('#jform_params_mega_config'),
			curconfig = null;
		...
		jmmconfig.val(jsonConfig);
	}

	toolbox_type = function () {
		return currentSelected.hasClass ('jux-nav-child') ? 'sub' : (currentSelected[0].tagName == 'DIV' ? 'col':'item');
	}

	hide_toolbox = function (show_intro) {
		$('#jux-admin-mm-tb .admin-toolbox').hide();
		currentSelected = null;
		if (megamenu && megamenu.data('nav_all')) megamenu.data('nav_all').removeClass ('selected');
		megamenu.find ('li').removeClass ('open');
		if (show_intro) {
			$('#jux-admin-mm-intro').show();
		} else {
			$('#jux-admin-mm-intro').hide();
		}
	}
	...
	show_toolbox = function (selected) {
		...
		var type = toolbox_type ();
		$('#jux-admin-mm-tool' + type).show();
		update_toolbox (type);

		$('#jux-admin-mm-tb').show();
	}

	update_toolbox = function (type) {
		if (!type) type = toolbox_type ();
		// remove all disabled status
		$('#jux-admin-mm-tb .disabled').removeClass('disabled');
		$('#jux-admin-mm-tb .active').removeClass('active');
...
//						var moduleTemplate = '<div class="module" ><h3></h3></div>';
						$.ajax({
							url: MOD_ACTION,
							data:{'task':'module', 'mid': value,'base_url':BASE_URL,'t':$.now()}
						}).done(function ( data ) {
							currentSelected.find('.mega-inner').html(data).find('input,select,textarea').removeAttr('name').removeAttr('class').removeAttr('id').removeAttr('required');
						});
					} else {
	...
	bindEvents = function (els) {
		...
		els.mouseover(function(event) {
			...
			megamenu.data('hovertimeout', setTimeout(function(){
				$this.addClass('hover');
			}, 100));
			event.stopPropagation();
		});
	...
	toggleinner = function(e){
		var itemsinner = $('.mega-inner');
		if(e){
			$.each(itemsinner,function(){
				var nav_child = $(this).closest('.jux-nav-child');
				if(nav_child.find('.mega-col-module').length == 0){
					if($(this).find('[data-type="category"]').length == 0){
						$(this).closest('.jux-nav-child').css('display','');;

					}
				}
				else { 
					if($(this).find('[data-type="category"]').length == 0){
						$(this).closest('.mega-col-nav').css('display','');
					}
				}
				var liinner = $(this).closest('li');
				liinner.children('a').addClass ('dropdown-toggle');
				liinner.addClass(liinner.data('level') == 1 ? 'dropdown' : 'dropdown-submenu');
			});
		}else{
			$.each(itemsinner,function(){
				var nav_child = $(this).closest('.jux-nav-child');
				if(nav_child.find('.mega-col-module').length == 0){
					if($(this).find('[data-type="category"]').length == 0){
						$(this).closest('.jux-nav-child').css('display','none');;
						var liinner = $(this).closest('li');
						liinner.removeClass('dropdown dropdown-submenu mega');
					}
				}
				else { 
					if($(this).find('[data-type="category"]').length == 0){
						$(this).closest('.mega-col-nav').css('display','none');
					}
				}
			});
		}
...
!function($){
	$.extend(AdminMegamenu, {
		prepare :function(){
			var JUXmegamenu = $('#jux-admin-mm-container').megamenuAdmin().find(':input').removeAttr('name');
		},
		initItemsControl:function(){
			var itemControl = $('#jform_params_show_items')
				,juxnav = $('ul.level0')
				,items = juxnav.find('[data-type="item"]');
			if(itemControl.find('input[checked=checked]').val() == 1){
				items.show();
				toggleinner(1);
			}else{
				items.hide();
			...
			itemControl.find('input[type=radio]').bind('click',function(){
				if($(this).val() == 1){
					juxnav.removeClass('hide-items');
					toggleinner(1);
					items.show();
				}else{
					juxnav.addClass('hide-items');
					toggleinner();
					items.hide();
				}
			...
			var item_num_control = $('#jform_params_show_num_items')
				,mega_num_item = $('.mega-num-item');
			if(item_num_control.find('input[checked=checked]').val() == 1){
				mega_num_item.removeClass('hide');
			}else{
				mega_num_item.addClass('hide');
			};
			
			item_num_control.find('input[type=radio]').bind('click',function(){
				if($(this).val() == 1){
					mega_num_item.removeClass('hide');
				}else{
					mega_num_item.addClass('hide');
		...
		initColModule:function(){
			var mega_col_module = $('.mega-col-module');
			mega_col_module.each(function(){
				if($(this).data('position')){
					var el = $(this);
					$.ajax({
						url: MOD_ACTION,
						data:{'task':'module', 'mid': $(this).data('position'),'base_url':BASE_URL,'t':$.now()}
					}).done(function ( data ) {
						var inner = $('.mega-inner',el);
						inner.html(data).find('input,select,textarea').removeAttr('name').removeAttr('class').removeAttr('id').removeAttr('required');;
					});
				}
			});
		},
		initRadioGroup: function(){
			...
			var juxmenu = $('.jux-admin-megamenu');

			juxmenu.find('.radio.btn-group label').addClass('btn');
			juxmenu.find('.btn-group label').unbind('click').click(function() {
				var label = $(this),
			...
			juxmenu.on('update', 'input[type=radio]', function(){
				if(this.checked){
					$(this)
						.closest('.btn-group')
			...
			juxmenu.find('.btn-group input[checked=checked]').each(function(){
				if($(this).val() == ''){
	...
	$(document).ready(function(){
		AdminMegamenu.initItemsControl();
		AdminMegamenu.initPreSubmit();
		AdminMegamenu.initRadioGroup();
		AdminMegamenu.initColModule();
		
		$('#JUXMClearConfig').click(function(e){
			if(confirm(JUXM_CLEAR_CONFIG)){
				$('#jform_params_mega_config').prop('value','');
				var form = document.adminForm;
				if(!form){
					return false;
				}
				form.task.value = 'module.apply';
				form.submit();
			};
		});
	});
