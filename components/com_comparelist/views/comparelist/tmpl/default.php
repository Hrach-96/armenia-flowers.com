...
<h3 class="module-title">
	...
	<div class="back-to-category" >
    	<a href="<?php echo $catURL ?>" class="button_back button reset2" title="<?php echo $categoryName ?>"><i class="fa fa-reply"></i><span></span></a>
	</div>
...
foreach ($prods as $product) 
		{
			...
			if (isset($product->customfields)) {
				foreach ($product->customfields as $field) {
							//print_r($field->custom_element.'</br>');
							if (($field->field_type == 'E' && $field->custom_element !== 'jmsyoutube')|| $field->field_type == 'P' || $field->field_type == 'G' ||  $field->field_type == 'I' || $field->field_type == 'B' || $field->field_type == 'Y') {
		...
		$rating = $ratingModel->getRatingByProduct($product->virtuemart_product_id);
			...
			if (!empty($products)) {
				...
				$table[$row][$col] .= '<div class="browseImage ">';
				 ...
				 $table[$row][$col] .= '<div class="lbl-box2">';
    ...
    if ((($stockhandle == 'disableit' or $stockhandle == 'disableadd') and (($product->product_in_stock - $product->product_ordered) < 1) || 
        ...
        $table[$row][$col] .= '<div class="sold">'.JText::_("DR_SOLD").'</div>';
       }elseif ((time() - $result < $dataNewOut ) && $lblnew ){ 
     $table[$row][$col] .= '<div class="new">'.JText::_('DR_NEW').'</div>';
     }     
     $table[$row][$col] .= '</div>';

         $table[$row][$col] .= '<div class="lbl-box">';
     if ($lbloffer && $product->prices['override'] == 1 && ($product->prices['product_price_publish_down'] > 0)){
   $table[$row][$col] .= ' <div class="limited">'.JText::_("DR_LIMITED_OFFER").'</div>';
       } elseif($discont >0 && $product->product_sales < $lblhotcount && $lblsale ) {
    $table[$row][$col] .= '<div class="discount">'.JText::_("DR_SALE").'</div>';
		} elseif ($lblhot && $product->product_sales > $lblhotcount) {
         ...
         $table[$row][$col] .= ' </div>';
				...
				$table[$row][$col] .= '<div class="comare_action">';
				 ...
				 if (!VmConfig::get('use_as_catalog', 0) and !empty($product->prices['salesPrice'])) {
					$table[$row][$col] .='<div class="addtocart-area2 proddet">';
					 if (($product->product_in_stock < 1) || ($product->product_in_stock < $product->min_order_level) || ($product->product_in_stock - $product->product_ordered) < $product->min_order_level) { 
						$table[$row][$col] .='<a class="addtocart-button" title="'.JText::_('COM_VIRTUEMART_CART_NOTIFY').'" href="'.JRoute::_('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id='.$product->virtuemart_product_id).'"><i class="fa fa-shopping-cart"></i>'.JText::_('COM_VIRTUEMART_CART_NOTIFY').'<span></span></a>';
										 } else { 
										...
										if ((!empty($product->customsChilds)) || (!empty($product->customfieldsCart))) { 
											...
											$table[$row][$col] .='<a class="addtocart-button" title="'.JText::_('DR_VIRTUEMART_SELECT_OPTION').'" href="'.JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$product->virtuemart_product_id).'"><i class="fa fa-shopping-cart"></i>'.JText::_('DR_VIRTUEMART_SELECT_OPTION').'<span></span></a>';
                                      	  	$table[$row][$col] .='</div>';
										} else { 
											$table[$row][$col] .='<div class="controls"><span class="box-quantity">';
											...
											$table[$row][$col] .='</span>';
											$table[$row][$col] .='<span class="quantity-controls js-recalculate">';
											$table[$row][$col] .='<button style="padding:0px;margin:0px;border:none;background:none;" type="button" class="quantity-controls quantity-plus"><i class="quantity-controls quantity-plus fa fa-caret-up"></i></button>';
											$table[$row][$col] .='<button style="padding:0px;margin:0px;border:none;background:none;" type="button" class="quantity-controls quantity-minus"><i class="quantity-controls quantity-minus fa fa-caret-down"></i></button>';
											$table[$row][$col] .='</span>';
											$table[$row][$col] .='</div></span>';
											$table[$row][$col] .='<div class="clear"></div>';
											$table[$row][$col] .='<span class="addtocart_button2">';
												$table[$row][$col] .='<button name="addtocart" type="submit" value="'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'" title="'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'" class="addtocart-button cart-click"><i class="fa fa-shopping-cart"></i>'.JText::_('COM_VIRTUEMART_CART_ADD_TO').'</button>';
											$table[$row][$col] .='</span>';
  				...
  				$table[$row][$col] .= '<div class="clear"></div>';
				$table[$row][$col] .= '<div class="remcompare"><a class="compare_del" title="remove"  onclick="removeCompare('.$product->virtuemart_product_id.');"><i class="fa fa-times"></i>'.JText::_('COM_COMPARE_REMOVE').'</a></div>';
				$table[$row][$col] .= '</div>';
